...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../part_b/B1.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../part_b/B2.png}
...
Observed results follow the same ordering as Part A. In B1, mean WCR is $0.810$ (camera-only), $0.050$ (multi-sensor permissive), and $0.000$ (mitigation). In B2 closed-transition subsets, CIR is $0.486$, $0.014$, and $0.000$, while LMS decreases from $22.429\,\mathrm{m}$ to $0.398\,\mathrm{m}$ and $0.012\,\mathrm{m}$ for the same architecture order. These values indicate that additional sensors reduce average risk, but contradiction-aware arbitration is required to suppress residual transition-window failures.
...
Across A1--B2, the dominant hazard is premature commitment under unresolved contradiction. The architecture-level comparison is therefore better interpreted as permissive commitment versus contradiction-aware commitment, rather than as sensor-count comparison alone. In both physics-driven (A1/A2) and rule-priority (B1/B2) corner cases, multi-sensor evidence improves average robustness but does not guarantee safety without explicit contradiction arbitration, uncertainty preservation, and conservative transition logic.
