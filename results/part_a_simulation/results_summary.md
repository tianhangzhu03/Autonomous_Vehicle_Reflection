...
Runs per cell: 40 seeds across three stacks (`camera_only`, `multimodal_permissive`, `mitigation`).

## Key Outcomes

- **A1 PBS (multi-modal permissive -> mitigation)**: 2.002 -> 1.220 (reduction 39.1%, Mann-Whitney p=2.78e-05).
- **A2 EVR (multi-modal permissive -> mitigation)**: 0.942 -> 0.098 (reduction 89.6%, Mann-Whitney p=1.44e-14).
...
- **A2 stop-time ratio**: camera-only=0.000, multi-modal permissive=0.000, mitigation=0.666.
